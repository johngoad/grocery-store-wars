#!/usr/bin/env python3
"""Matcher v2.2 — first-word primary, ALL candidates scored, multi-word fallback."""
...
log("=== Matcher v2.2: no candidate capping ===")
...
for p in tw_products:
    for code in _get_upcs(p):
        tw_by_upc[code] = p
log(f"  UPC index: {len(tw_by_upc)} codes")
...
log(f"  First-word index: {len(tw_by_first_word)} words")

# All-words index (fallback — only used when first-word < 3 candidates)
tw_by_all_words = defaultdict(list)
for p in tw_products:
    words = _get_all_significant_words(_normalize(p["name"]))
    for w in set(words):
        tw_by_all_words[w].append(p)
log(f"  All-words index: {len(tw_by_all_words)} words")
...
for i, iga in enumerate(iga_products):
    if (i + 1) % 1000 == 0:
        ...
        log(f"  {i+1}/{len(iga_products)}... ({matched} m, {fallback_used} fb, {elapsed:.0f}s)")
    ...
    if best_confidence < UPC_MATCH_CONFIDENCE:
        # Collect candidates: first-word primary
        candidate_ids = set()
        candidates = []
        ...
        # Fallback: multi-word if < MIN candidates
        if primary_count < MIN_FALLBACK_CANDIDATES:
            fallback_used += 1
            for w in _get_all_significant_words(iga_name):
        ...
        # Score ALL candidates — no input cap
        cand_map = {}
        cand_names = []
        priority_names = []
        other_names = []
        ...
        for c in candidates:
            ...
            if priority > 0:
                priority_names.append(cn)
            else:
                other_names.append(cn)

        # Priority first, then others. process.extract scores all.
        cand_names = priority_names + other_names

        # Get top results. process.extract handles large inputs fine.
...
elapsed = time.time() - t0
log(f"  {len(iga_products)}/{len(iga_products)}... ({matched} m, {fallback_used} fb, {elapsed:.0f}s)")

if statements:
    log(f"  Writing {len(statements)} matches...")
...
log(f"\n{'='*50}")
log(f"v2.2 RESULTS: {matched}/{len(iga_products)} ({pct:.1f}%)")
