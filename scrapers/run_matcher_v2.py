#!/usr/bin/env python3
"""Matcher v2 — standalone script, run with project venv."""
...
for i, iga in enumerate(iga_products):
    if (i + 1) % 1000 == 0:
        log(f"  {i+1}/{len(iga_products)}... ({matched} matched)")
    ...
    if best_confidence < UPC_MATCH_CONFIDENCE:
        ...
        for w in _get_significant_words(iga_name):
        ...
        all_cand_names = (priority_candidates + other_candidates)[:TOP_N_CANDIDATES * 3]
...
log(f"  {len(iga_products)}/{len(iga_products)}... ({matched} matched)")
...
log(f"\nRESULTS: {matched}/{len(iga_products)} matches ({pct:.1f}%)")
